		...
		visitor: {
			ExpressionStatement(path, state) {
				...
				if (!state.data.insideRestify) {
					...
					if (path.node.expression.left.name == 'ROUTED') state.data.sections.routed = path.node.expression.right;else if (path.node.expression.left.name == 'AFTER') state.data.sections.after = path.node.expression.right;else if (path.node.expression.left.name == 'CLOSE') state.data.sections.close = path.node.expression.right;
					...
					parseExpr(state, path, (state, expr) => {
						if (expr.type == 'StringLiteral') {
							...
							state.data.currentRoute = (0, _babelGenerator2.default)(expr.argument).code;
							state.data.sections.routes[state.data.currentRoute] = {
								path: expr.argument,
								assign: []
							};
				...
				} else throw path.buildCodeFrameError("invalid expression");
...
function initState() {
	return {
		...
		createServerTemplate: (0, _babelTemplate2.default)(`var VARNAME = restify.createServer(OPTIONS);`),
		serverPreTemplate: (0, _babelTemplate2.default)(`VARNAME.pre(MIDDLEWARE);`),
		serverUseTemplate: (0, _babelTemplate2.default)(`VARNAME.use(MIDDLEWARE);`),
		serverOnTemplate: (0, _babelTemplate2.default)(`VARNAME.on(EVENT, MIDDLEWARE);`),
		serverMethodTemplate: (0, _babelTemplate2.default)(`VARNAME.METHOD(OPTIONS, HANDLER);`)
	};
}

function stateToCode(state, path) {
	var code = [state.data.createServerTemplate({
	...
	for (let key in state.data.sections.routes) {
		...
		for (let i = 0, len = assign.length; i < len; i++) {
			var node = assign[i];
			if (node.left.type != 'Identifier') throw "route entry assignment must have identifier on left hand side";
			...
			if (left == 'name') {
				...
				if (curName) options.properties.push(t.objectProperty(t.identifier("name"), curName));
				if (curVersion) options.properties.push(t.objectProperty(t.identifier("version"), curVersion));
...
function parseSequence(state, path, func, allowedTypes = undefined) {
	for (let i = 0, len = path.node.expression.expressions.length; i < len; i++) {
		...
		if (allowedTypes && !allowedTypes.includes(expr.type)) throw path.buildCodeFrameError("parseSequence: invalid expression");
...
function parseExpr(state, path, func, allowedTypes = undefined) {
	if (path.node.expression.type == 'SequenceExpression') {
		...
		if (allowedTypes && !allowedTypes.includes(path.node.expression.type)) throw path.buildCodeFrameError("parseExpr: invalid expression");
